...
recipies_template = env.get_template('recipies.html')
...
recipies_html = recipies_template.render(posts=posts_metadata, tags=tags)
submit_html = submit_template.render()

with open('output/recipies.html', 'w') as file:
    file.write(recipies_html)

with open('output/submit.html', 'w') as file:
    file.write(submit_html)
...
for post in POSTS:
    ...
    post_file_path = 'output/{slug}.html'.format(slug=post_metadata['slug'])
